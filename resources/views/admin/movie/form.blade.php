...
<div class="container">
    ...
    <form action="{{ route('movies.api_import.store') }}" method="POST" id="ophim_form">
        @csrf
        <div class="form-group mt-3">
            <label for="api_url">URL API Ophim</label>
            <input type="url" class="form-control" id="ophim_api_url" name="api_url" placeholder="Nhập URL API Ophim" value="{{ old('api_url') }}" required autofocus>
        </div>
        <button type="submit" class="btn btn-primary">Thêm phim từ Ophim</button>
    </form>

    <form action="{{ route('movies.api_import.storekk') }}" method="POST" id="kkphim_form" style="display: none;">
        @csrf
        <div class="form-group mt-3">
            <label for="api_url">URL API KKphim</label>
            <input type="url" class="form-control" id="kkphim_api_url" name="api_url" placeholder="Nhập URL API KKphim" value="{{ old('api_url') }}" required autofocus>
        </div>
